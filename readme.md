...
java 中的整型数据运算符有几个特殊应用方法：%  判断是否能整除 ，<< 、>> 左移和右移实现乘2和除2操作，^ 异或操作实现数据变换和恢复
...
任务：识别出文件中数值；为文件新增一列，总计，并在对应行写入对应的数值；新增一行，平均价格，并在同一行最右边，添加数值

提示：从文件中读入数据，可以采用将输入重定向为文件完成



#### D04 交互式输入输出([Code](D04/))
...
任务：读入文件，将数据文件中的数据显示在控件中；写入文件，将读入的数据计算得出总计并写入文件

2、设计一个界面，读入一个算术表达式

增加一个命令按钮“单步”，单步操作对应将运算表达式分解，并按照运算的顺序，每次执行一步，将运算操作数和运算符用控件 显示在窗口上。可以使用作业一中已经创建完成的类来完成其中的分解工作。增加一个按钮“存储”
